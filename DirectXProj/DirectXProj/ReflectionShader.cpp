...
void ReflectionShader::initShader(WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Setup the description of the reflection buffer that is in the vertex shader.
...
void ReflectionShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* reflectionTexture, const XMMATRIX &reflectionMatrix_)
{
	...
	trefl = XMMatrixTranspose(reflectionMatrix_);
	...
	deviceContext->Map(reflectionBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	reflectionPtr = (ReflectionBufferType*)mappedResource.pData;
	reflectionPtr->reflectionMatrix = reflectionMatrix_;// worldMatrix;
