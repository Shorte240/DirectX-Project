...
DisplacementDepthShader::DisplacementDepthShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	// Quad tessellation
	// Uses the same vs, hs and ps as the tessellation depth shader
	// With a different ds for height map calculations
	initShader(L"tessellation_vs.cso", L"tessellation_hs.cso", L"displacementDepth_ds.cso", L"tessellationDepth_ps.cso");
}
...
void DisplacementDepthShader::initShader(WCHAR * vsFilename, WCHAR * psFilename)
{
...
void DisplacementDepthShader::initShader(WCHAR * vsFilename, WCHAR * hsFilename, WCHAR * dsFilename, WCHAR * psFilename)
{
...
void DisplacementDepthShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, float tessFactor, float height, XMFLOAT3 camPos)
{
	...
	// Set shader texture and sampler in the domain shader.
